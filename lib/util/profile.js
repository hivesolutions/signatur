...
/**
 * Validates the calligraphy object of a profile definition.
 *
 * @param {Object} calligraphy The calligraphy object to validate.
 * @returns {Array} An array of error messages (empty if valid).
 */
const validateCalligraphy = calligraphy => {
    const errors = [];
    if (typeof calligraphy !== "object" || calligraphy === null) {
        return ["calligraphy must be an object"];
    }
    if (calligraphy.line_width !== undefined) {
        errors.push(...validatePositiveNumber(calligraphy.line_width, "calligraphy.line_width"));
    }
    return errors;
};
...
const validateProfile = profile => {
    ...
    if (profile.sku !== undefined && typeof profile.sku !== "string") {
        errors.push("sku must be a string");
    }

    if (profile.category !== undefined && typeof profile.category !== "string") {
        errors.push("category must be a string");
    }

    if (profile.default_font !== undefined && typeof profile.default_font !== "string") {
        errors.push("default_font must be a string");
    }
 ...
 *
 * @param {String} profileText The profile JSON payload as text.
 * @param {String} inspirationsText The optional inspirations JSON
 *     payload as text, ignored when empty.
 * @returns {Object} An object with parsed profile, inspirations,
 *     and an errors array (empty if valid).
 */
const validateProfileSubmission = function(profileText, inspirationsText) {
    ...
    if (inspirationsText && inspirationsText.trim()) {
        ...
        if (inspirations) {
            const inspirationErrors = validateInspirations(inspirations);
 ...
 *
 * @param {Object} profile The parsed profile object.
 * @param {String} editTarget The id of the profile being edited,
 *     or an empty value for create mode.
 * @param {String} directoryPath The absolute path of the on disk
 *     profiles directory.
 * @returns {Array} An array of error messages (empty if valid).
 */
const validateProfileFilesystem = async function(profile, editTarget, directoryPath) {
    ...
    // refuses any edit target that does not match the validated
    // identifier pattern so a crafted form field cannot escape
    // the profiles directory through `path.join` interpolation
    if (editTarget && !PROFILE_ID_PATTERN.test(editTarget)) {
        errors.push(`invalid edit target "${editTarget}"`);
        return errors;
    }
...
module.exports = {
    ...
    ASSET_FILENAME_PATTERN: ASSET_FILENAME_PATTERN,
    ...
    validateProfileFilesystem: validateProfileFilesystem
};
