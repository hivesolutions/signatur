...
const validateMachine = machine => {
    ...
    if (machine.viewport_height === undefined) {
        ...
        errors.push(
            ...validatePositiveNumber(machine.viewport_height, "machine.viewport_height")
        );
    }
    return errors;
};
...
const validateText = text => {
    ...
    if (
        text.vertical_align !== undefined &&
        !VALID_VERTICAL_ALIGNS.includes(text.vertical_align)
    ) {
        errors.push(
            `text.vertical_align must be one of: ${VALID_VERTICAL_ALIGNS.join(", ")}`
        );
    }
    return errors;
};
