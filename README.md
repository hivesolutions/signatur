...
| editor state    |        no         | `font`, `font_size`, `font_size_mode`, `zoom`, `margins`, `rulers`, `crosshair`, `keyboard`, `guidelines`, `caret`, `overflow` are written to the URL by the viewport only, and only while `.viewer-container` is mounted on the page so they never leak into `/welcome`, `/gateway`, `/settings` or any other downstream section. |
...
| `font_size`      | `float` | `null`  | Manual font size in profile units; applied to both the slider and the number input.                                                                                                                                                                                                      |
...
| `overflow`       | `str`   | `"0"`   | When `"1"` allows the lines to wrap past the engraving area; otherwise a character that would not fit its line is refused with a warning, a font size increase stops at the largest size the text still fits and the text is trimmed to fit whenever the margins or the template change. |
