...
| `overflow`       | `str` | `"0"`   | When `"1"` allows the lines to wrap past the engraving area; otherwise a character that would not fit its line is refused with a warning and the text is trimmed to fit whenever the font size, the margins or the template change. |
