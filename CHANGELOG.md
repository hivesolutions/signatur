...
* Per print job font shipping on the confirm engraving modal that rewrites every `Cool Emojis` multifont entry into the underlying engraving glyph name through the active `coolemojis.mapping.json`, collects the set of unique font names actually referenced by the rewritten payload and resolves each into a base64 `.f3s` payload through a new `GET /settings/fonts/resolve?names=...` route, attaching the resulting `{ name: base64 }` map to the gravo print payload as the `extra_fonts` field that colony print and gravo pilot install for the duration of the engraving session ([#56](https://github.com/hivesolutions/signatur/issues/56))
