...
* `.github/dependabot.yml` enabling weekly automated update PRs for the `docker` ecosystem (the `Dockerfile` base image digest) and for the `github-actions` ecosystem (every `uses:` reference in the workflow files), with the `chore` commit prefix so the generated PRs match the project's Conventional Commits convention out of the box
* `Dockerfile` base image is now pinned to the multi arch manifest list digest (`node:20-bookworm-slim@sha256:2cf067cfed83…`) instead of a floating tag so the build is reproducible across runs and so Dependabot has a concrete artifact to bump on its weekly cadence
* `Dockerfile` rewritten as a slim two stage build on `node:20-bookworm-slim` so the runtime image no longer carries the full `hivesolutions/ubuntu_dev:latest` toolchain (compilers, jdk, gradle, etc.) and the AGPL flagged dev packages that came with it; the builder stage installs only the headers needed to compile `pstoedit-4.01` from source under `--prefix=/opt/pstoedit`, the final stage installs only the genuine runtime tools (`ghostscript`, `inkscape`) and copies the `/opt/pstoedit` tree across with `PATH` and `LD_LIBRARY_PATH` extended to find the binary and its driver libraries; node dependencies are installed with `npm install --omit=dev` so `mocha`, `nodemon`, `eslint`, `prettier`, `sharp` and the other `devDependencies` no longer ship in the production image; legacy `ADD` directives become explicit `COPY` lines with `package.json` copied before the source so editing a `.js` file no longer busts the `npm install` cache layer; the container now runs as a dedicated `signatur` user with UID/GID 10001 (`/app` chowned to it) instead of root, satisfying the Docker Scout non root user check; the legacy `ENV KEY VALUE` syntax is replaced by `ENV KEY=VALUE` per the current Docker reference, the image label `version` is bumped to `2.0` to reflect the rewrite, and `CMD` invokes `node` from `PATH` instead of the hardcoded `/usr/bin/node` so future base image relocations of the node binary do not break the entrypoint
